...
is_button_camera_settings_visible = False
...
class RealtimeCam(ft.UserControl):
    ...
    async def update_kontras(self, value):
        value = int(value)
        self.cap.set(cv2.CAP_PROP_CONTRAST, value)

    async def update_layar(self, value):
        value = int(value)
    ...
    async def update_cam(self):
        while self.running:
            ...
            # Buat objek CLAHE
            # Atur grid size di sini
            clahe = cv2.createCLAHE(clipLimit=value, tileGridSize=(8, 8))

            # Terapkan CLAHE ke citra yang sudah di-blur
            ...
            # Update citra pada kontrol video real-time
...
async def main(page: ft.Page):
    ...
    battery_status_text = ft.Text("", weight=ft.FontWeight.BOLD, size=20)
    battery_icon = ft.Icon(name=ft.icons.BATTERY_FULL,
                           size=60, color=ft.colors.GREEN)
    charging_icon = ft.Icon(name=ft.icons.BOLT, size=40,
                            color=ft.colors.YELLOW, visible=False)

    async def update_battery_status():
        while True:
            percent, plugged = await get_battery_status()
            battery_status_text.value = f"{percent}%"
            charging_icon.visible = plugged
    ...
    async def button_pencerahan(e):
        ...
        container_brightness.visible = is_button_pencerahan_visible
        await page.update_async()
    ...
    async def button_camera_settings(e):
        global is_button_camera_settings_visible
        is_button_camera_settings_visible = not is_button_camera_settings_visible
        container_camera_settings.visible = is_button_camera_settings_visible
        await page.update_async()
    ...
    async def slider_kontras_changed(e):
        await video_cam.update_kontras(e.control.value)
        await page.update_async()
    ...
    capture_icons_camera_settings = ft.Image(
        src="settings.png",
        width=40,
        height=40,
        fit=ft.ImageFit.CONTAIN,
    ...
    container_brightness = ft.Container(
        content=ft.Column(
            [
                ft.Column([
                    ft.Text("KECERAHAN LAYAR",
    ...
    container_camera_settings = ft.Container(
        content=ft.Column(
            [
                ft.Column([
                    ft.Text("KECERAHAN KAMERA",
                            style=ft.TextThemeStyle.TITLE_SMALL),
                    ft.Slider(width=150, min=1, max=100,
                              divisions=100, label="{value}%", on_change=slider_kecerahan_changed),
                    ft.Text("KONTRAS KAMERA",
                            style=ft.TextThemeStyle.TITLE_SMALL),
                    ft.Slider(width=150, min=1, max=100,
                              divisions=100, label="{value}%", on_change=slider_kontras_changed),
                ])
            ],
        ),
        visible=is_button_camera_settings_visible
    )
    ...
    container_clahe = ft.Container(
        content=ft.Column(
            [
                ft.Column([
                    ft.Text("CLAHE Clip Limit",
                            style=ft.TextThemeStyle.TITLE_SMALL),
                    ft.Slider(width=150, min=1, max=100,
                              divisions=100, label="{value}", on_change=slider_change_cliplimit),
                ])
            ],
        ),
                        ...
                        content=ft.Column(
                            [
                                ...
                                ft.Column([
                                    ...
                                    container_clahe,
                                    ft.IconButton(
                                        content=capture_icons_camera_settings,
                                        icon=capture_icons_camera_settings,
                                        icon_size=30,
                                        on_click=button_camera_settings,
                                        bgcolor="WHITE"
                                    ),
